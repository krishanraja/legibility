  ...
  component: () => (
    <article className="space-y-6">
      ...
      <p className="text-lg text-muted-foreground">
        Legibility exposes the same tools over the Model Context Protocol. Agents discover the
        server, call <span className="font-mono">read_product</span>, and pay per call in USDC on
        Base.
      </p>
...
Accept: application/json, text/event-stream`}
      ...
      <p className="text-muted-foreground">
        Tool calls without a valid payment header return{" "}
        <span className="font-mono">402 Payment Required</span> with payment instructions. Currently
        settled on <strong>Base Sepolia</strong> (testnet). Mainnet flip on GA.
      </p>
      <pre className="rounded-md border border-hairline bg-surface p-5 font-mono text-sm overflow-x-auto">
