  ...
  component: () => (
    <article className="space-y-6">
      ...
      <p className="text-lg text-muted-foreground">
        Limits are enforced per API key, per minute and per day. Cached reads count at one-tenth the
        weight of a live extraction.
      </p>
            ...
            <tr key={row[0]}>
              {row.map((c, i) => (
                <td
                  key={i}
                  className={
                    i === 0 ? "px-4 py-3 text-foreground" : "px-4 py-3 text-muted-foreground"
                  }
                >
                  {c}
                </td>
...
x-ratelimit-reset:     1734457200`}
      ...
      <p className="text-muted-foreground">
        ...
        <span className="font-mono"> retry-after</span> in seconds. The MCP surface returns the same
        status. x402-paid calls share the per-recipient bucket and are subject to the same caps to
        keep cache hit rates honest.
      </p>
    </article>
  ),
});
