          ...
          mainEntity: [
            {
              ...
              acceptedAnswer: {
                "@type": "Answer",
                text: "JSON-LD and OpenGraph first, a Shopify path for Shopify stores, barcode databases for GTINs, and an unblocker for bot-hostile retailers. The source method is on every response.",
              },
            },
            {
              "@type": "Question",
              name: "Why a confidence score?",
              acceptedAnswer: {
                "@type": "Answer",
                text: "Because product pages lie. Legibility returns a calibrated confidence per field and overall, where the 0.7 gate means about 70 percent likely correct. You decide the threshold, and you only pay for reads that clear the gate.",
              },
            },
            {
              "@type": "Question",
              ...
              acceptedAnswer: {
                "@type": "Answer",
                text: "Price is a band with an as_of timestamp and a source count. Legibility does not present a single live number as a guarantee.",
              },
            },
            {
              "@type": "Question",
              ...
              acceptedAnswer: {
                "@type": "Answer",
                text: "An open micropayment standard. Agents pay per call in USDC on Base, with no signup and no key rotation. Base Sepolia testnet in beta.",
              },
            },
            {
              "@type": "Question",
              name: "Why not Diffbot?",
              acceptedAnswer: {
                "@type": "Answer",
                text: "Diffbot returns a typed object but does not score confidence per field, does not stamp cost in the response, and has no MCP server or x402 surface, so an agent cannot discover it and pay on its own.",
              },
            },
            {
              "@type": "Question",
              name: "Why not roll your own with Firecrawl and GPT?",
              acceptedAnswer: {
                "@type": "Answer",
                text: "You can. You also own the schema, the cache policy, the confidence rubric, the price-band semantics, the barcode merge, the MCP server, and the x402 settlement. Legibility is that work, finished, behind one call.",
              },
            },
          ],
        }),
...
function buildLines(r: HeroRead): Line[] {
  ...
  if (price)
    lines.push({
      text: `    "price":  { "low": ${price.low}, "high": ${price.high}, "currency": "${price.currency}", "n_sources": ${price.n_sources} },`,
      delay: 70,
    });
  if (attrs) lines.push({ text: `    "attributes": ${JSON.stringify(attrs)},`, delay: 60 });
  lines.push({
    text: `    "availability": ${JSON.stringify(p.availability ?? "unknown")}`,
    delay: 55,
  });
  ...
  if (r.legibility_id)
    lines.push({ text: `  "legibility_id": ${JSON.stringify(r.legibility_id)},`, delay: 55 });
  ...
  return (
    <form onSubmit={submit} className="grid grid-cols-1 gap-3 sm:grid-cols-[1fr_1fr_auto]">
      ...
      <Input
        ...
        className="bg-surface sm:col-span-3"
      />
    </form>
  );
}
...
function Index() {
          ...
          <div className="mx-auto grid max-w-[1280px] grid-cols-1 gap-12 px-6 py-20 lg:grid-cols-[1fr_1.1fr] lg:py-28">
            <div className="flex flex-col justify-center">
              ...
              <h1 className="font-display mt-6 text-5xl leading-[1.05] text-balance text-foreground sm:text-6xl lg:text-[4.25rem]">
                ...
                <span className="block italic text-stone mt-3 text-balance max-w-[22ch]">
                  Reading the product page is where they still break.
                </span>
              </h1>
              <p className="mt-8 max-w-xl text-base leading-relaxed text-muted-foreground sm:text-lg">
                Legibility reads it for them. One call turns a URL, a barcode, or a fuzzy name into
                a typed product object. Confidence per field, cost stamped in the response, payable
                by an agent.
              </p>
              <div className="mt-10 flex flex-wrap items-center gap-3 font-mono text-sm">
                <a
                  href="#waitlist"
                  className="inline-flex items-center gap-2 rounded-md bg-signal px-5 py-2.5 font-medium text-background hover:opacity-90"
                >
                  Request access <span aria-hidden>→</span>
                </a>
                <a
                  href="/docs"
                  className="inline-flex items-center gap-2 rounded-md border border-hairline px-5 py-2.5 text-foreground hover:border-foreground"
                >
                  Read the docs
                </a>
              </div>
            ...
            <div className="lg:pl-4">
              <div className="overflow-hidden rounded-md border border-hairline bg-surface shadow-[0_30px_60px_-30px_rgba(14,27,44,0.25)]">
                ...
                <div className="flex items-center justify-between gap-4 border-t border-hairline px-4 py-2.5 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
                  <span>
                    A real response, captured live from the request above. URL, name, or barcode:
                    same typed object.
                  </span>
            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
              <div>
                ...
                <p className="mt-3 text-muted-foreground">
                  You're writing the buy-flow for an agent. You don't want to maintain a JSON-LD
                  parser, a headless Chrome fleet, a barcode lookup, and a price cache. You want one
                  call that returns the same shape every time.
                </p>
              </div>
              <div>
                <div className="font-display text-2xl text-foreground">Agents</div>
                <p className="mt-3 text-muted-foreground">
                  Discover the MCP server, call{" "}
                  <span className="font-mono text-signal">read_product</span>, pay per call in USDC
                  over x402. No account. No key rotation. The cost is in the response so you can
            ...
            <div className="divide-y divide-hairline border-y border-hairline">
              {TOOLS.map((t) => (
                ...
                >
                  ...
                  <span
                    className={`rounded-sm border px-2 py-0.5 text-[10px] uppercase tracking-widest ${t.live ? "border-signal/50 text-signal" : "border-hairline text-muted-foreground"}`}
                  >
        ...
        <section className="border-b border-hairline">
          <div className="mx-auto max-w-[1280px] px-6 py-24 grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-10">
            <div>
              ...
              <p className="mt-4 text-sm text-muted-foreground">
                Diffbot has the data. Firecrawl has the render. Neither was built for an agent to
                call and pay on its own.
              </p>
            </div>
              ...
              {[
                {
                  name: "Free",
                  price: "$0",
                  desc: "1,000 trusted reads/mo · no card",
                  feats: ["Single secret API key", "REST + MCP", "Email support"],
                },
                {
                  name: "Starter",
                  price: "$29",
                  desc: "5,000 trusted reads · $0.01 overage",
                  feats: ["Higher rate limits", "All confidence levels", "Priority email"],
                  featured: true,
                },
                {
                  name: "Growth",
                  price: "$199",
                  desc: "50,000 trusted reads · $0.005 overage",
                  feats: ["Highest rate limits", "Slack channel", "SLA"],
                },
              ].map((p) => (
                <div
                  key={p.name}
                  className={`rounded-md border p-7 ${p.featured ? "border-signal bg-background" : "border-hairline bg-background"}`}
                >
                  ...
                  <div className="font-display text-5xl text-foreground mt-3">
                    {p.price}
        ...
        <section id="mcp" className="border-b border-hairline">
          <div className="mx-auto grid max-w-[1280px] grid-cols-1 gap-10 px-6 py-24 lg:grid-cols-[280px_1fr]">
            <div>
              ...
              <p className="mt-4 text-sm text-muted-foreground">
                Same core, two surfaces. Devs hold a key. Agents discover the server and pay per
                call.
              </p>
            </div>
  ...
  -d '{ "gtin": "${DEMO_GTIN}" }'`}
              ...
              <div className="rounded-md border border-hairline bg-surface p-6">
                <div className="flex items-center gap-2 font-mono text-xs uppercase tracking-widest text-muted-foreground">
                  <span className="dot" style={{ background: "var(--verified)" }} /> agent · MCP +
                  x402
                </div>
                <pre className="mt-5 overflow-x-auto font-mono text-xs leading-relaxed text-foreground">
                  {`${MCP_URL}
  ...
  → { …object, cost_usd: 0.008 }`}
            ...
            <div className="divide-y divide-hairline border-y border-hairline">
              {[
                ...
                [
                  ...
                  <>
                    Yes.{" "}
                    <a href="/takedown" className="text-signal underline">
                      File here
                    </a>
                    . We honor it within 24h.
                  </>,
                ],
              ].map(([q, a], i) => (
        ...
        <section id="waitlist" className="border-b border-hairline">
          <div className="mx-auto max-w-[820px] px-6 py-24 text-center">
            ...
            <p className="mt-4 text-muted-foreground">
              Sign in with a magic link, create a key, and make your first call. Free to start, no
              card required.
            </p>
